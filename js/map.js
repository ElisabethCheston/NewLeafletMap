  require([
    ...
    function(
      ...
      var popupTrailheads = {
        "title": "Kitespot",
        "content": "<b>Name:</b> {TRL_NAME}<br><b>Description:</b> {CITY_JUR}<br><b>Weather:</b> {X_STREET}<br><b>Parking:</b> {PARKING}<br><b>Elevation:</b> {ELEV_FT} ft"
      }
...
for (var i = 0; i < kitespots.length; i++) {
    kitespots = new L.marker([kitespots[i][2],kitespots[i][1]])
...
for (var i = 0; i < kitespots.length; i++) {
 ...
 /*
 var map = L.map('mapid').setView([63.45, 17.52], 5);
